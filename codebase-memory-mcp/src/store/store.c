...
struct cbm_store {
    ...
    sqlite3_stmt *stmt_find_nodes_by_prefix;
...
int cbm_store_find_nodes_by_prefix(cbm_store_t *s, const char *project, const char *prefix,
                                    cbm_node_t **out, int *count) {
    if (!s) {
        *out = NULL;
        *count = 0;
    ...
    char pattern[CBM_SZ_512];
    snprintf(pattern, sizeof(pattern), "%s%%", prefix);
    return find_nodes_generic(s, &s->stmt_find_nodes_by_prefix,
                              "SELECT id, project, label, name, qualified_name, file_path, "
                              "start_line, end_line, properties FROM nodes "
                              "WHERE project = ?1 AND qualified_name LIKE ?2;",
                              project, pattern, out, count);
}
